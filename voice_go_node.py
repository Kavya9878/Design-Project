...
print("Available microphones:", mic_list) # <-- added for voice recognition

# Change this index to match your built-in mic from the list above
BUILTIN_MIC_INDEX = 0
...
class RI_Challenge(Node):
    def __init__(self):
        ...
        self.target_room_publisher_ = self.create_publisher(String, '/target_room', 10)
    ...
    def voice_listener(self):
        ...
        mic = sr.Microphone()
        self.get_logger().info("Voice command listener started. Say 'go to room X'.")
        ...
        while rclpy.ok():
            try:
                ...
                if "stop" in command:
                    self.get_logger().info("Stop command detected — cancelling navigation.")
                    self.navigator.cancelTask()
                    continue

                # 解析命令
                for key in locations.keys():
                    if key in command.replace(" ", ""):  # 支持 room1 / room 1
                        msg = String()
                        msg.data = key
                        self.target_room_publisher_.publish(msg)
                        break
    ...
    def detect(self, label, frame_count=1, min_detections=1, timeout=10.0):
        ...
        return len(self.detection_queue) >= min_detections
    ...
    def wait_until_navigation_complete(self, timeout=240.0):
        ...
        while not self.navigator.isTaskComplete():
            ...
            if elapsed_time > timeout:
                self.get_logger().warn(f"Navigation timeout after {timeout} seconds. Cancelling task.")
                self.navigator.cancelTask()
                break
